...
### TRIGを押さずにCONTROL BYTESが出る場合

`CONTROL BYTES`は受信した読取りデータに制御文字が含まれるため、在庫イベントにしなかった表示です。本体リセットだけで出る場合は、Unitの設定・読取り停止への応答がデータとして届いた可能性があります。ブート復帰の`RECOVERY HOLD`や電源異常とは分けて確認します。

既知の成功応答だけで構成された受信は読取り結果として扱いません。未知の応答やコードと混ざった受信は、引き続き検査で拒否します。表示が続く場合はTRIGを操作していないことと発生のタイミングを記録し、コード本文を除いた診断ログを確認してください。未送信イベントを消したり、応答部分を手作業で除いて在庫へ登録したりしないでください。この症状の原因確定と、実機での応答除外の確認は未完了です。
