...
既知の成功応答だけで構成された受信は読取り結果として扱いません。未知の応答やコードと混ざった受信は、引き続き検査で拒否します。表示が続く場合はTRIGを操作していないことと発生のタイミングを記録し、コード本文を除いた診断ログを確認してください。未送信イベントを消したり、応答部分を手作業で除いて在庫へ登録したりしないでください。原因と修正後の実機動作は未確認です。
