...
- TRIG未操作の`CONTROL BYTES`が読取りデータの検査エラーであり、ブート復帰・電源異常とは別だと説明できる。[切り分け手順](setup.md#trigを押さずにcontrol-bytesが出る場合)から発生タイミングを確認できる。
- 既知のUnit成功応答だけの受信では読取り状態・在庫キューを更新せず、未知・混在した受信は従来の検査を維持することを確認できる。実機の原因や修正確認を合成試験から推定しない。
